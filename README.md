...
- **キー自動検出**（手動指定も可能）
- メジャー / マイナー / dim / aug / sus、7th 系（7・m7・M7・m7(b5)・dim7）に対応
- **テンション**（add9・9・M9・m9）と **分数コード**（`C/E` など）も検出
...
| `--seventh` | 7th 系（7・m7・M7・m7(b5)・dim7）も検出対象に加える |
| `--full`    | さらにテンション（add9・9・M9・m9）も検出（誤検出も増える） |
...
- テンション・分数コードを拾いたい → `--full`（誤検出も増える点に注意）
- とにかく安定させたい → `--simple`
